...
def get_json_from_api(srch_str, max_pages = None):
  ...
  while has_next_page:
    if not max_pages or current_page < max_pages:
      ...
      break
    page_info_str = "(Current Page: " + str(current_page) + " / " + str(total_pages) + ")"
    print("    Searching for movie with string filter", "'"+srch_str+"'", page_info_str, "    ", end="\r")
    url_str = "https://www.rottentomatoes.com/napi/search/all?after={}&searchQuery={}&type=movie&f=null".format(after, parsed_search_str)
...
def filter_movies(test_string, movies_list):
  ...
  for movie in movies_list:
    ...
    if all(s in movie['name'].lower().replace(" ", "") for s in test_string.lower().split()):
      ...
      if ("value" in movie['criticsScore']) and (movie['criticsScore']['value']):
        ...
        if ("score" in movie['audienceScore']) and (movie['audienceScore']['score']):
...
def pick_best_movie(movies_list):
  ...
  _hcs_list = list(filter(lambda m: (int(m['criticsScore']['value']) >= _hcs), movies_list))
  ...
  _has_list = list(filter(lambda m: (int(m['audienceScore']['score']) >= _has), _hcs_list))
...
def format_result(srch_str, movie):
  ...
  movie_info_str += "Critic Score: " + str(movie['criticsScore']['value']) + "%" + "\n"
...
def main_app():
  ...
  parser.add_argument('strings', metavar='search_query_string', type=str, nargs='+',
                      help='search query')
  parser.add_argument('--load-json-file', dest='json_load_file_path', type=str,
                      help='provide a json file instead of querying the api')
  parser.add_argument('--save-json-file', dest='json_save_file_path', type=str,
                      help='save a json file after querying the api')
  parser.add_argument('--max-dl-pages', dest='max_dl_pages',
  ...
  if not any(c.isalpha() for c in search_string) and \
     not any(c.isnumeric() for c in search_string):
    print("ERROR: search_string: -->{}<-- does not contain any letters or numbers".format(search_string))
    sys.exit()

  if args.json_load_file_path and args.json_save_file_path:
    print("ERROR: Please only use `--load-json-file` OR `--save-json-file` NOT BOTH")
    sys.exit()
  ...
  if len(filtered_movie_list) == 0:
    ...
    print('HINT: try rephrasing the query. E.g. "god father ii" instead of "god father 2"')
    sys.exit() 
